        ...
        if os.path.exists(self.graph_ls_file):
            ...
            # memory friendly
                ...
                for i in range(len(self.keys)):
                    ...
                    with open(self.graph_dic_path + self.path_marker + str(dic['key']), 'wb') as f:
                        pickle.dump(dic, f)
...
class TransTRAP(nn.Module):
    ...
    def __init__(self, p_max=12, b_max=18, in_feat_b=1280, in_feat_p=84, d_model=200, d_ff=512, d_k=128, d_v=128, n_heads=4, n_layers=3,
                 dropout=0.20, d_FC_layer=100, n_FC_layer=2, n_tasks=1 ,d_graph_layer=200, batch_size=256):
        ...
        self.fc = FC(self.d_model*2, self.d_FC_layer, self.n_FC_layer, dropout, self.n_tasks)
