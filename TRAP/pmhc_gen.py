...
def epi_featurizer_esm_sequence(esm_model, esm_batch_converter, sequence, device):
...
def epi_featurizer_esm(epi, pmhc, inter_dict1, inter_dict2, inter_dict3, pmhc_atoms_aves,
    ...
    esm_info_th = torch.tensor(epi_featurizer_esm_sequence(esm_model, esm_batch_converter, sequence, device)['x'], dtype=torch.float)  # len = 1280
    # print(res_aves)
...
def epi_feature_esm(system_path, dis_threshold1, dis_threshold2, dis_threshold3, epi_max,
                    esm_model, esm_batch_converter, device='cpu'):
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for system_idx in os.listdir(struc_path):
            ...
            epi_feat = epi_feature_esm(system_path, dis_threshold1, dis_threshold2, dis_threshold3, esm_model, esm_batch_converter, dev)
